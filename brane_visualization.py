...
def groupbyplot(
    ...
    selection = (
        selection.to_frame(name="size")
        .reset_index()
        .rename(columns=columns, inplace=False)
    )
    ...
    if kind == "piechart":
        ...
        ax = selection.plot(kind="bar", y="Count", x=column_name, title=title, rot=0)
    else:
